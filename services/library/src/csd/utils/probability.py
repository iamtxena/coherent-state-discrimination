...
def _find_success_probability_and_input_codeword_for_one_output_codeword(
        ...
        output_codeword: CodeWord) -> Tuple[float, CodeWord]:
    findings = [(codeword.success_probability, codeword.input_codeword)
                for codeword in codewords_success_probabilities if codeword.output_codeword == output_codeword]
    if len(findings) <= 0:
...
def compute_maximum_likelihood(batch_success_probabilities: List[List[CodeWordSuccessProbability]],
    ...
    for output_codeword in output_batch.codewords:
        (max_success_probability,
         associated_input_codeword) = _find_max_success_probability_of_all_input_codewords_for_one_output_codeword(
