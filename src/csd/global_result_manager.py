...
class GlobalResultManager(ABC):
        ...
        with open(alpha_file, "r") as f:
            ...
            alpha_results = [
                GlobalResult(
                    ...
                    number_modes=int(row[3]),
                    time_in_seconds=float(row[4]),
                    squeezing=strtobool(row[5] if len(row) >= 6 else "False"),
                    number_ancillas=int(row[6] if len(row) >= 7 else 0),
                    helstrom_probability=float(row[7] if len(row) >= 8 else 0.0),
                    homodyne_probability=float(row[8] if len(row) >= 9 else 0.0),
                    best_success_probability=float(row[9] if len(row) >= 10 else 0.0),
                    best_helstrom_probability=float(row[10] if len(row) >= 11 else 0.0),
                    best_homodyne_probability=float(row[11] if len(row) >= 12 else 0.0),
                    best_codebook=json.loads(row[12]) if len(row) >= 13 else [],
                    best_measurements=row[13] if len(row) >= 14 else [],
                    best_optimized_parameters=row[14] if len(row) >= 15 else {},
                )
                for row in reader
                if len(row) > 0
            ]
        ...
        with open(self._global_results_file, "r") as f:
            ...
            self._global_results = [
                GlobalResult(
                    ...
                    number_modes=int(row[3]),
                    time_in_seconds=float(row[4]),
                    squeezing=strtobool(row[5]),
                    number_ancillas=int(row[6]),
                    helstrom_probability=float(row[7]),
                    homodyne_probability=float(row[8]),
                    best_success_probability=float(row[9]),
                    best_helstrom_probability=float(row[10]),
                    best_homodyne_probability=float(row[11]),
                    best_codebook=json.loads(row[12]) if len(row) >= 13 else [],
                    best_measurements=row[13] if len(row) >= 14 else [],
                    best_optimized_parameters=row[14] if len(row) >= 15 else {},
                )
    ...
    def plot_success_probabilities(
        self, save_plot: bool = False, interactive_plot: bool = False, best_codebook: Optional[bool] = False
    ) -> None:
        if (
            not hasattr(self, "_selected_global_results")
            or not hasattr(self, "_number_modes")
            or not hasattr(self, "_alphas")
        ):
            self.load_results()
    ...
    def plot_distances(self, save_plot=False, interactive_plot: bool = False) -> None:
        if (
            not hasattr(self, "_selected_global_results")
            or not hasattr(self, "_number_modes")
            or not hasattr(self, "_alphas")
    ...
    def plot_bit_error_rates(self, save_plot=False, interactive_plot: bool = False) -> None:
        if (
            not hasattr(self, "_selected_global_results")
            or not hasattr(self, "_number_modes")
            or not hasattr(self, "_alphas")
    ...
    def plot_computation_times(self, save_plot=False, interactive_plot: bool = False) -> None:
        if (
            not hasattr(self, "_selected_global_results")
            or not hasattr(self, "_number_modes")
            or not hasattr(self, "_alphas")
    ...
    def plot_modes_probs(
        ...
        best_codebook: Optional[bool] = False,
    ) -> None:
        if (
            not hasattr(self, "_selected_global_results")
            or not hasattr(self, "_number_modes")
                ...
                for number_ancilla in self._number_ancillas:
                    for squeezing_option in squeezing_options:
                        ...
                        for result in self._global_results:
                            if (
                                result.alpha == alpha
                                and result.number_modes == number_mode
                                and result.number_ancillas == number_ancilla
                                and result.squeezing == squeezing_option
                                and result.distance_to_helstrom_probability > max_distance
                            ):
                                max_result = result
