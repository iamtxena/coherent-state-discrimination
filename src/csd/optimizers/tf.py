...
from csd.utils.util import (
    ...
    CodebookLearningStepsLogInformation,
...
class TFOptimizer(ABC):
    def __init__(
        self,
        nparams: int = 1,
        learning_steps: LearningSteps = LearningSteps(default=150, high=300, extreme=500),
    ...
    def optimize(
        ...
        for step in range(current_learning_steps):
            ...
            reset = self._print_time_when_necessary(
        ...
        return OptimizationResult(
            optimized_parameters=[param.numpy() for param in parameters],
    ...
    def _prepare_tf_board(self, current_alpha: float) -> None:
        ...
        train_log_dir = (
            f"logs/gradient_tape/modes/{self._number_modes}/{current_time}" f"_alpha_{np.round(current_alpha, 2)}"
        )
        ...
        self._train_params = [
            tfMetrics.Mean(f"train_param_{param_name}", dtype=tf.float32) for param_name in self._params_name
        ]

    def _tf_optimize(
        self, cost_function: Callable, parameters: List[Variable]
    ) -> Tuple[EagerTensor, List[Variable], List[CodeWordSuccessProbability]]:
    ...
    def _print_time_when_necessary(
        self,
        learning_steps: int,
        init_time: float,
        step: int,
        ...
        if reset and codebook_log_info is not None:
            ...
            print_codebook_learning_steps_log(codebook_log_info=codebook_log_info, learning_log_info=learning_log_info)
        return reset

    def _print_optimized_parameters_for_tf_backend_only(
        self, step: int, optimized_parameters: List[EagerTensor]
    ) -> bool:
        print_every_n_times = 50
        if (step + 1) % print_every_n_times == 0:
            logger.debug(
                "Learned parameters value at step {}: {}".format(
