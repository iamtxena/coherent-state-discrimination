...
class CSD(ABC):
    ...
    def execute_testing_circuit_one_codeword(
        ...
        for index, codebook in enumerate(codebooks.codebooks):
            ...
            top5_testing_codebooks.add(
                potential_best_codebook=BestCodeBook(
                    codebook=self._current_codebook,
            ...
            logger.debug(f"pSucc: {one_alpha_success_probability.numpy()} " f"codebook_size:{self._codebook_size}")
    ...
    def execute_given_polar_codes(
        ...
        """Run an experiment for the same batch with the given codebooks"""
        self._alphas = [alpha_value]
        self._cutoff_dim = cutoff_dimension
        self._architecture["number_modes"] = number_modes
        self._architecture["squeezing"] = squeezing
        self._run_configuration = RunConfiguration(
            {
                "run_backend": Backends.TENSORFLOW,
                "optimization_backend": OptimizationBackends.TENSORFLOW,
        ...
        return self._train_and_test_polar_codes(
            one_alpha=alpha_value,
    ...
    def _train_and_test_polar_codes(
        self,
        one_alpha: float,
        number_modes: int,
        polar_codebooks: CodeBooks,
        ...
        start_time = time()
        one_alpha_start_time = time()
        self._alpha_value = one_alpha
        self._current_batch = Batch(
            size=0, word_size=number_modes, all_words=False, input_batch=polar_codebooks.codebooks[0]
        )
        self._batch_size = self._current_batch.size
        ...
        codebooks = polar_codebooks
        self._all_codebooks_size = codebooks.size
        logger.debug(
            f"Optimizing for alpha: {np.round(self._alpha_value, 2)} "
            f"with {self._all_codebooks_size} polar codebooks."
        )
        ...
        for index, codebook in enumerate(codebooks.codebooks):
            ...
            self._current_codebook_index = index
            self._current_codebook = codebook
            logger.debug(f"current polar codebook: {self._current_codebook}")
            self._codebook_size = len(codebook)
            self._engine = self._create_engine()
            self._current_codebook_log_info = CodeBookLogInformation(
                alpha_value=np.round(self._alpha_value, 2),
            ...
            logger.debug(
                ...
                f"polar codebook_size:{self._codebook_size}"
            )
        ...
        self._update_result(
            ...
            one_alpha_success_probability=self._get_succ_prob(None, training_one_alpha_optimization_result),
        ...
        self._write_result(
            alpha=one_alpha,
            one_alpha_start_time=one_alpha_start_time,
            success_probability=self._get_succ_prob(None, training_one_alpha_optimization_result),
            helstrom_probability=training_average_ideal_helstrom_probability_all_codebooks,
            homodyne_probability=training_average_ideal_homodyne_probability_all_codebooks,
        ...
        logger.debug(
            ...
            f"BEST TESTING success probability: {top5_testing_codebooks.first.success_probability}\n"
            f"Best TRAINING POLAR codebook: {top5_training_codebooks.first}\n"
            f"Best TESTING POLAR codebook: {top5_testing_codebooks.first}\n\n"
        )
    ...
    def execute(self, configuration: RunConfiguration) -> ResultExecution:
        ...
        return self._train_and_test(
            training_result=training_result, testing_result=testing_result, random_words=(not self._backend_is_tf())
        )

    def _get_succ_prob(
        self, one_alpha_success_probability: Union[EagerTensor, None], one_alpha_optimization_result: OptimizationResult
    ) -> float:
        return (
            one_alpha_success_probability.numpy()
            if one_alpha_success_probability is not None
    ...
    def _update_best_and_worst_success_probability(
        self,
        ...
        best_codebook: List[CodeWord],
    ) -> Tuple[float, float, List[CodeWord]]:
    ...
    def _test_for_one_alpha_one_codebook(
        self, optimized_parameters: List[float]
    ...
    def _write_result(
        ...
        if testing:
            directory_name = (
                f"./circuit_tex/alpha_{np.round(alpha, 2)}"
    ...
    def _create_optimization(self) -> Optimize:
        ...
        if self._run_configuration is None:
            raise ValueError("Run configuration not specified")
        return Optimize(
            opt_backend=self._run_configuration["optimization_backend"],
    ...
    def _train_for_one_alpha_one_codebook(self) -> OptimizationResult:
        ...
        logger.debug(
            f"Trained for alpha: {np.round(self._alpha_value, 2)}"
    ...
    def _create_engine(self) -> Union[Engine, TFEngine]:
        ...
        if self._backend_is_tf():
            return TFEngine(
                ...
                engine_backend=Backends.TENSORFLOW,
                options={
        ...
        return Engine(
            number_modes=circuit.number_modes,
            engine_backend=self._run_configuration["run_backend"],
            options={"cutoff_dim": self._current_cutoff},
        )
    ...
    def _set_plot_title(
        self, batch_size: int, plays: int, modes: int, layers: int, squeezing: bool, ancillas: int
    ) -> str:
        if self._run_configuration is None:
            raise ValueError("Run configuration not specified")

        return (
            f"backend:{self._run_configuration['run_backend'].value}, "
    ...
    def execute_all_backends_and_measuring_types(
        self,
        backends: List[Backends] = [Backends.FOCK, Backends.GAUSSIAN, Backends.TENSORFLOW],
        measuring_types: Optional[List[MeasuringTypes]] = [MeasuringTypes.PROBABILITIES],
        optimization_backend: OptimizationBackends = OptimizationBackends.SCIPY,
    ) -> List[ResultExecution]:
        ...
        if required_probability_execution:
            self._probability_results = self._execute_with_given_backends(
                backends=backends,
                measuring_type=MeasuringTypes.PROBABILITIES,
        ...
        if required_sampling_execution:
            self._sampling_results += self._execute_with_given_backends(
                backends=backends, measuring_type=MeasuringTypes.SAMPLING, optimization_backend=optimization_backend
            )
        return self._probability_results + self._sampling_results

    def _execute_with_given_backends(
        self, backends: List[Backends], measuring_type: MeasuringTypes, optimization_backend: OptimizationBackends
    ) -> List[ResultExecution]:
    ...
    def plot_success_probabilities(
        ...
        measuring_types: Optional[Union[List[MeasuringTypes], None]] = None,
    ) -> None:
