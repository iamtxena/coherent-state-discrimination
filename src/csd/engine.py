...
class Engine(ABC):
    ...
    def __init__(
        ...
        self._engine = sf.Engine(backend=self._backend.value, backend_options=options)
    ...
    def _apply_guess_strategy_move_noise_to_ancillas(
        ...
        codewords_success_probabilities: List[CodeWordSuccessProbability],
    ) -> CodeWordSuccessProbability:
        ...
        success_probability = self._compute_guessed_codeword_probability(
            guessed_codeword, codewords_success_probabilities
        )

        return CodeWordSuccessProbability(
            input_codeword=max_success_probability_codeword_selected.input_codeword,
    ...
    def _create_codeword_from_last_output_modes(
        self, output_codeword: CodeWord, codeword_to_guess: CodeWord
    ) -> CodeWord:
        return CodeWord(word=output_codeword.word[-codeword_to_guess.size :])

    def _compute_guessed_codeword_probability(
        self, guessed_codeword: CodeWord, codewords_success_probabilities: List[CodeWordSuccessProbability]
        ...
        return tf.math.add_n(
            [
                codeword_success_probabilities.success_probability
                for codeword_success_probabilities in codewords_success_probabilities
                if self._create_codeword_from_last_output_modes(
                    output_codeword=codeword_success_probabilities.output_codeword, codeword_to_guess=guessed_codeword
                )
                == guessed_codeword
            ]
        )

    def _error_when_codeword_to_guess_is_larger_than_output_codeword(
        self, output_codeword: CodeWord, codeword_to_guess: CodeWord
    ):
        if output_codeword.size < codeword_to_guess.size:
            raise ValueError(
                f"Output codeword size: {output_codeword.size} MUST NOT be"
                f"larger than codeword to guess size: {codeword_to_guess.size}"
            )

    def _max_probability_codeword(
        self, codewords_success_probabilities: List[CodeWordSuccessProbability]
    ) -> CodeWordSuccessProbability:
        ...
        zero_prob = (
            sum(
                [
                    1
                    for read_value in [
                        self._run_circuit(circuit=circuit, options=options).samples[0][0] for _ in range(shots)
                    ]
                    if read_value == 0
                ]
            )
        ...
        return [
            CodeWordSuccessProbability(
                ...
                guessed_codeword=CodeWord(
                    size=options["input_codeword"].size, alpha_value=options["input_codeword"].alpha
                ),
    ...
    def _run_circuit_probabilities(
        ...
        return self._compute_fock_probabilities_for_all_codewords(
            state=result.state,
            input_codeword=options["input_codeword"],
    ...
    def _get_fock_prob_indices_from_modes(
        self, output_codeword: CodeWord, cutoff_dimension: int
    ) -> List[CodeWordIndices]:
        ...
        return [
            CodeWordIndices(
                ...
                indices=self._convert_word_to_fock_prob_indices(codeword=output_codeword, cutoff_dim=cutoff_dimension),
            )
            for output_codeword in output_codewords
        ]

    def _compute_fock_probabilities_for_all_codewords(
        self, state: BaseState, input_codeword: CodeWord, output_codeword: CodeWord, cutoff_dim: int
    ) -> List[CodeWordSuccessProbability]:
        ...
        return [
            CodeWordSuccessProbability(
                ...
                success_probability=self._compute_fock_prob_one_word(
                    state=state, fock_prob_indices_one_word=codeword_indices.indices
                ),
                counts=0,
            )
    ...
    def _run_circuit(self, circuit: Circuit, options: EngineRunOptions) -> Result:
        ...
        return self._engine.run(
            ...
            args=self._parse_circuit_parameters(circuit=circuit, options=options),
            shots=options["shots"],
        )
    ...
    def _convert_word_to_fock_prob_indices(self, codeword: CodeWord, cutoff_dim: int) -> List[List[int]]:
        ...
        minus_groups = [
            p for p in itertools.product(dimensions_more_than_0_photons, repeat=codeword.number_minus_alphas)
        ]
