...
def evaluate():
    ...
    for i in range(2 ** NUM_MODES):
        ...
        pad_length = NUM_MODES - len(string_in_binary)
    ...
    for codeword in codewords:
        t_previous_predictions = np.random.normal(size=NUM_MODES * NUM_VARIABLES)
        ...
        if np.allclose(codeword, -prediction_of_final_layer):
            n_correct += 1


    logger.info(f"Accuracy: {n_correct/len(codewords):0.4f}.")
